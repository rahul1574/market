...
function PasswordRecoverycost() {
    ...
    const sendOTP = async () => {
      try {
        const response = await axios.post('http://localhost:80/api/send-otp-cost.php', { email});
    ...
    const verifyOTP = async () => {
      try {
        const response = await axios.post('http://localhost:80/api/verify_otp-cost.php', { email, otp });
        setMessage(response.data.message);
        if (response.data.status === 1) {
    ...
    const updatePassword = async () => {
      ...
      try {
        const response = await axios.post('http://localhost:80/api/update_password-cost.php', { email, password });
        setMessage(response.data.message);
        if (response.data.status === 1) {
      ...
      <div id='dashboard' style={{background:'white',height:'750px',display:'flex',flexDirection:'row',flexWrap:'wrap',alignItems:'center',justifyContent:'space-around'}}>
        ...
        {step === 1 && (
          ...
          <div style={{display:'flex',flexDirection:'row',flexWrap:'wrap',alignItems:'center',justifyContent:'space-evenly'}}>
            <div style={{display:'flex',flexDirection:'column',flexWrap:'wrap',alignItems:'center'}}>
              <h2 >Password Recovery Vendor</h2>
              <h3>Send OTP</h3>
        ...
        {step === 3 && (
          ...
          <div style={{display:'flex',flexDirection:'row',flexWrap:'wrap',alignItems:'center'}}>
            ...
            <div style={{display:'flex',flexDirection:'row',flexWrap:'wrap',alignItems:'center'}}>
             ...
             />
              <button onClick={updatePassword} style={{height:'30px',width:'150px',margin:'10px'}}>Update Password</button>
              <p>{message}</p>
              <div style={{height:'400px',width:'300px',margin:'10px'}}>
                <img 
        ...
        {step === 4 && (
          <div style={{display:'flex',flexDirection:'column',flexWrap:'wrap',alignItems:'flex-start',justifyContent:'space-evenly'}}>
            <h1>{message}</h1>
            <Link to='/signin'><button style={{height:'30px',width:'150px',margin:'10px'}}>Back to login</button></Link>
          </div>
        )}
      </div>
    );
...
export default PasswordRecoverycost